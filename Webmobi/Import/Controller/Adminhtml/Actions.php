...
abstract class Actions extends \Magento\Backend\App\Action
{
    ...
    {
        $_preparedActions = array('index', 'grid', 'new','test', 'edit', 'editPrice','savePrice', 'save', 'delete', 'config', 'massStatus', 'massDelete');
        $_action = $this->getRequest()->getActionName();
		if($this->_idKey=='price_id' && $_action== 'edit'){
			$_action="editPrice";
		}
		if($this->_idKey=='price_id' && $_action== 'save'){
			$_action="savePrice";
		}
       ...
       protected function _testAction()
    {
		...
		// echo $model->getUrl().$model->getParent();
		$data = new \Webmobi\Import\Helper\Data($model->getUrl(),null);
		if($model->getId())
			$pd=$data->xml_node_to_product($model->getId());
 ...
 /**
     ...
     * @return void
     */
    public function _editPriceAction()
    {
		$models=array();
		$feed =$this->_objectManager->create("Webmobi\Import\Model\ResourceModel\Feed");
		 $id = (int)$this->getRequest()->getParam('feed_id');
		 $collection = $feed->getAllPrice($id); //Get Collection of module data

		foreach($collection as $p){
			$model =$this->_objectManager->create("Webmobi\Import\Model\Price");
			$price=$model->load($p['price_id']);
			$models[]=$price;
			}
        ...
        $this->_getRegistry()->register('current_model', $models);
        $this->_getRegistry()->register('feed_id', $id);
...
*/
    ...
    {
		if($this->_idKey=='field_id')
			return "Edit XML Mapping";
...
/**
     ...
     * @return void
     */
    public function _savePriceAction()
    {
        ...
        $resultRedirect = $this->resultRedirectFactory->create();
		 try {
			if(isset($data["price"])){
				foreach($data["price"] as $price){
					if(isset($price["price_id"])){
						...
						$model->load($price["price_id"]);	
						if($model){
							$model->setPricemin($price["pricemin"]);
							$model->setPricemax($price["pricemax"]);
							$model->setAdded($price["percentage"]);
							$model->save();
						}	
						if(empty($price["pricemin"]) && empty($price["pricemax"]) && empty($price["percentage"]))
							$model->delete();
					}
					if(!isset($price["price_id"]) && !empty($price["percentage"]) ){
							$model = $this->_objectManager->create($this->_modelClass);
							$model->setPricemin($price["pricemin"]);
							$model->setPricemax($price["pricemax"]);
							$model->setAdded($price["percentage"]);
							$model->setFeed($data["feed_id"]);
							$model->save();
					}
				}
			}
            ...
            // } else {
                $this->_redirect('*/*/edit', ["feed_id" => $data["feed_id"]]);
            // }
            return;
    ...
    public function _saveAction()
    {
        ...
        try {
            ...
            $this->messageManager->addSuccess(__($model->getName().' has been saved.'));
    ...
    protected function _getModel($load = true)
    {
    	if (is_null($this->_model)) {
			...
			if($this->_idKey=='field_id'){
				$id = (int)$this->getRequest()->getParam('feed_id');
				 $this->_model->load($id,'feed_id');
					if(!$this->_model->getId()){
						$this->_model->setFeed($id);
						$this->_model->save();
					 }
				}
